...
def display_instructions():
    ...
    Welcome! You are participating in a research study exploring how humans supervise AI agents performing complex tasks. In this simulation, you have tasked an AI agent with selling a specific camera (e.g., a used Sony Alpha a7 III) on your behalf in an online marketplace. The AI interacts with potential buyers via text chat based on the instructions you (hypothetically) provided.
    ...
    3.  In the text box provided, answer the following:
        ...
        *   **What, if anything, could have been done differently during the agent's execution (e.g., better monitoring, different agent capabilities)?**
...
def end_session():
    ...
    try:
        # Save session data (reflections and stories)
...
def main():
    ...
    if 'story_manager' not in st.session_state: # Check a primary key like story_manager
        ...
        st.session_state.current_chapter_index = 0 # Use 0-based index internally
        st.session_state.session_id = st.session_state.story_manager.session_id # Get session ID from manager
        st.session_state.input_key_counter = 0 # Initialize the counter here
        st.session_state.prolific_id = "" # Initialize prolific ID
        st.session_state.ended = False # Initialize ended state
        st.session_state.save_status = None # Initialize save status
    
    # Sidebar content - show this regardless of started state
    with st.sidebar:
        st.image("https://img.icons8.com/fluency/96/camera.png", width=80)
        
        # Only show Quick Tips
        st.markdown("""
        <div style='background-color: #eef5f9; padding: 20px; border-radius: 10px; margin-top: 20px;'>
            <h3 style='color: #1f77b4; margin-bottom: 10px;'>Reflection Tips</h3>
            <ul style='color: #333; list-style-type: none; padding-left: 0;'>
                <li style='margin-bottom: 10px;'>🤔 Consider the agent's goal vs. its actions.</li>
                <li style='margin-bottom: 10px;'>📜 Think about specific instructions you might have given.</li>
                <li style='margin-bottom: 10px;'>⚙️ Could monitoring or agent design changes help?</li>
                <li style='margin-bottom: 10px;'>✅ Explain your reasoning clearly.</li>
            </ul>
        </div>
        """, unsafe_allow_html=True)
    ...
    if st.session_state.started:
        ...
        <ul>
            <li>Was the agent's behavior <strong>appropriate</strong> or <strong>inappropriate</strong>? Why?</li>
            <li>What could have been done differently during <strong>task definition</strong> 
            <li>What could have been done differently during <strong>agent execution</strong>
        </ul>
        </div>
        """, unsafe_allow_html=True)
        ...
        if submit_button:
            if user_reflection and len(user_reflection.strip()) > 10:
                ...
                st.warning("Please provide a reflection (at least 10 characters) before proceeding.")
