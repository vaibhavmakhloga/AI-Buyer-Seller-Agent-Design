...
class StoryManager:
    def __init__(self, session_id=None, prolific_id=None):
        ...
        self.feature_rankings = [] # Keep for now
        self.current_chapter = 0 # Track the chapter the user is currently on
        
        # Initialize MongoDB connection
        try:
            self.mongo = MongoManager()
            print(f"MongoDB connection established for session {self.session_id}")
        except ConnectionError as e:
            print(f"Error initializing MongoDB connection: {e}")
            # Since we're not handling file fallback, app will need to handle MongoDB failures
            self.mongo = None

    def set_prolific_id(self, prolific_id):
        """Set the Prolific ID for this session."""
        self.prolific_id = prolific_id
    ...
    def save_session(self):
        ...
        if not self.mongo:
            print("Warning: MongoDB connection not available, unable to save session")
            return False
        ...
        try:
            # Save to MongoDB (no file backup)
            success = self.mongo.save_session(session_data)
            return success
